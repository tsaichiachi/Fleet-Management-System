...
const InterestPaymentTable = ({
  ...
  const fetchInitialData = async () => {
    try {
      const response = await requestHttp("payInterest/getPayInterest", {
        method: "POST",
  ...
  const handleSaveClick = async () => {
    try {
      const { payDate, expireDate, disable } = editedRow;
      ...
      if (editingRowId === "new") {
        const response = await requestHttp("payInterest/addPayInterest", {
          method: "POST",
          data: dataToSave,
        });
...
export default InterestPaymentTable;
