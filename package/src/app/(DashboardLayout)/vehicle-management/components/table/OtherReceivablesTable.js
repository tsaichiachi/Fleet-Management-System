...
const OtherReceivablesTable = ({
  ...
  const fetchInitialData = async () => {
    try {
      const response = await requestHttp("otherLendMoney/getOtherLendMoney", {
        method: "POST",
  ...
  const handleSaveClick = async () => {
    try {
      const { lendDate, expireDate, disable } = editedRow;
      ...
      if (disable === "1") {
        ...
        console.log("計算的沖銷月份:", targetBillYearMonth);
      }
      ...
      if (editingRowId === "new") {
        const response = await requestHttp("otherLendMoney/addOtherLendMoney", {
          method: "POST",
          data: dataToSave,
        });
        ...
        if (disable === "1") {
          ...
          response = await requestHttp("otherLendMoney/voidAndRebillToMonth", {
            method: "POST",
            data: {
              id: editedRow.id,
              targetBillYearMonth,
          ...
          // 不作廢時調用原本的 updateOtherLendMoney API
          response = await requestHttp(
            "otherLendMoney/updateOtherLendMoney",
            {
              method: "POST",
              data: dataToSave,
            }
  ...
  return (
    <Box sx={{ overflow: "auto", width: "100%" }}>
      ...
      <Table aria-label="simple table" sx={{ whiteSpace: "nowrap", mt: 2 }}>
        <TableHead>
          <TableRow>
            {["日期(YYY-MM-DD)", "應收金額", "備註", "作廢", "操作"].map(
              (header, index) => (
                <TableCell key={index}>
        ...
        <TableBody>
          {editingRowId === "new" && (
            <TableRow>
              {["lendDate", "amount", "note"].map((field, index) => (
                <TableCell key={index}>
                  {field === "type" ? (
                    <Select
...
export default OtherReceivablesTable;
